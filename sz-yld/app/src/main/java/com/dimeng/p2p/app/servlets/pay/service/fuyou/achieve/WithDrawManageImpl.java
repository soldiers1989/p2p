...
public class WithDrawManageImpl extends AbstractEscrowService implements WithDrawManage
{
        ...
        throws Throwable
    {
        ...
        try (Connection connection = getConnection())
        {
            ...
            {
                // 用户的往来账户
                T6101 wlzh = this.selectT6101(connection, accountId, T6101_F03.WLZH);
                if (amount.compareTo(wlzh.F06) > 0)
                {
                    throw new LogicalException("账户余额不足");
                }
                // 用户的锁定账户
                T6101 sdzh = selectT6101(connection, accountId, T6101_F03.SDZH);
                if (sdzh == null)
                {
                ...
                mchnt_txn_ssn = MchntTxnSsn.getMts(FuyouTypeEnum.YHTX.name());
                t6501.F10 = mchnt_txn_ssn;
                // 插入订单
                int orderId = insertT6501(connection, t6501);
                t6503 = new T6503();
                t6503.F02 = accountId;
                if ("ON".equals(configureProvider.format(FuyouVariable.FUYOU_CHAREFEE_ONOFF)))
                {
    ...
    {
        try (PreparedStatement pstmt =
            connection.prepareStatement("INSERT INTO S65.T6501 SET F02 = ?, F03 = ?, F04 = CURRENT_TIMESTAMP(),F05 = CURRENT_TIMESTAMP(), F07 = ?, F08 = ?, F09 = ?, F10 = ?",
                PreparedStatement.RETURN_GENERATED_KEYS))
        {
            pstmt.setInt(1, entity.F02);
            pstmt.setString(2, entity.F03.name());
            pstmt.setString(3, entity.F07.name());
            pstmt.setInt(4, entity.F08);
            pstmt.setInt(5, entity.F09);
            pstmt.setString(6, entity.F10);
            pstmt.execute();
    ...
    {
        try (PreparedStatement pstmt =
            connection.prepareStatement("INSERT INTO S61.T6130 SET F02 = ?, F03 = ?, F04 = ?, F06 = ?, F07 = ?, F08 = CURRENT_TIMESTAMP(), F09 = ?, F13=?, F14 = CURRENT_TIMESTAMP()",
                PreparedStatement.RETURN_GENERATED_KEYS))
        {
            pstmt.setInt(1, entity.F02);
    ...
    /**
     * 描述:插入流水记录 作者:wangshaohua 创建时间：2015年4月29日
     * 
     * @param connection
     * @param entity
     * @return
    ...
    {
        try (PreparedStatement pstmt =
            connection.prepareStatement("INSERT INTO S61.T6102 SET F02 = ?, F03 = ?, F04 = ?, F05 = CURRENT_TIMESTAMP(), F06 = ?, F07 = ?, F08 = ?, F09 = ?",
                PreparedStatement.RETURN_GENERATED_KEYS))
        {
            pstmt.setInt(1, entity.F02);
            pstmt.setInt(2, entity.F03);
        ...
        throws Throwable
    {
        int adminId = 1000;
        try (PreparedStatement ps =
            connection.prepareStatement("SELECT T7110.F01 FROM S71.T7110 WHERE F02 LIKE'%admin%' LIMIT 1"))
        {
            try (ResultSet rs = ps.executeQuery())
            {
                 ...
                 * 此处必须去掉
                 * else {
                    adminId = serviceResource.getSession().getAccountId();
                }*/
            }
        }
        return adminId;
            ...
            StringBuilder sql =
                new StringBuilder(
                    "SELECT T6501.F01 AS F01,T6501.F03 AS F02,T6501.F04 F03,T6501.F05 AS F04,T6501.F06 AS F05,T6501.F10 AS F06,T6503.F03 AS F07,T6503.F08 AS F08,T6110.F02 AS F09 "
                        ...
                        + "WHERE T6501.F02='10002' OR T6501.F02='10008' ");
