 ...
 */
@AchieveVersion(version = 20150530)
@ResourceAnnotation
public class HFBidPrePaymentExecutor extends TenderPrepaymentExecutor
{
    
    public HFBidPrePaymentExecutor(ResourceProvider resourceProvider)
    {
        ...
        throws Throwable
    {
        ...
        String merPriv = "";
        if (ps != null && ps.get("orderIdsStr") != null)
        {
        ...
        String ptAccount = configureProvider.getProperty(HuifuVariable.HF_CUST_ID);
        
        String Version = HuiFuConstants.Version.VERSION_20;
        String CmdId = "Repayment";
        String MerCustId = ptAccount;
        ...
        String TransAmt = t6521.F06.setScale(2, BigDecimal.ROUND_HALF_UP).toString();
        String Fee = fee.setScale(2, BigDecimal.ROUND_HALF_UP).toString();
        
        String InCustId = t6521.F02 == getPTID(connection) ? ptAccount : usrCustId(connection, t6521.F02);
        ...
        String FeeObjFlag = "I";
        String BgRetUrl = configureProvider.format(HuifuVariable.HF_PREPAYMENT);
        ...
        logger.info(String.format("提前还款订单%s 请求参数：%s", orderId, map.toString()));
        ...
        try
        {
            ...
            try
            {
                    ...
                    && response.getStatusLine().getStatusCode() == HttpStatus.SC_OK)
                {
                    ...
                    logger.info(String.format("提前还款订单%s 返回参数：%s", orderId, rst));
                }
