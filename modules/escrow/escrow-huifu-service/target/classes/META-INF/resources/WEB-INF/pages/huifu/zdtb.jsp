...
<%@ page import="com.dimeng.p2p.user.servlets.huifu.HFZdtbClose" %>
<%@ page import="com.dimeng.p2p.user.servlets.huifu.HFZdtb" %>
...
<div class="main_bg clearfix">
       ...
       <div style="width:360px; margin:auto">
           ...
           <a id="zztb" href="javascript:void(0)"  class="btn07 fl">开启自助式自动投资</a>
                        ...
                        <tr>
                            ...
                            <td align="center">
                                <%if (auto.autoSetStatus.name().equals(AutoSetStatus.TY.name())) { %>
                               	<a class="blue" target="_bank" href="<%=controller.getURI(request, HFZdtb.class) %>" >开启</a> 
                                <%} else { %> 
                              	<a class="blue" target="_bank" href="<%=controller.getURI(request, HFZdtbClose.class) %>">关闭</a> 
                                <%} %> 
                                <a class="blue" href="javascript:void(0);" onclick="deleteZdtb(<%=auto.id%>,'<%=auto.autoSetStatus%>')">删除</a>
                            </td>
                        </tr>
                        <%
                            }
                ...
                <div class="mt30 lh30">
                    ...
                    1、投资进度达到<%=DoubleParser.parse(configureProvider.getProperty(SystemVariable.AUTO_BIDING_END_PROGRESS)) * 100 %>
                    ...
                    %的剩余金额。<br/>
                    2、单笔投资金额若超过该标借款总额的<%=DoubleParser.parse(configureProvider.getProperty(SystemVariable.AUTO_BIDING_MAX_PERCENT)) * 100 %>
                    %，则按照该标借款总额的<%=DoubleParser.parse(configureProvider.getProperty(SystemVariable.AUTO_BIDING_MAX_PERCENT)) * 100 %>
                    %金额投资。<br/>
                    3、借款用户在获得借款时会自动关闭自动投资，以避免借款被用作自动投资资金。<br/>
                    4、投标排序规则如下：<br/>
						a）每个用户可以开启一个自动投标规则，可以在规则里面设置自动投标条件。<br/>
						b）每个规则投标序列按照上一次自动投标时间先后进行排序。<br/>
						c）每个规则每个标限制自动投标10次，投标后，排到队尾。<br/>
						d）如果余额不足，停止投资，余额充足后，继续投资。<br/>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="dialog" id="zztb_dialog" style="display: none;width: 520px;">
        ...
        <div class="content">
            <form target="_bank" action="<%=configureProvider.format(URLVariable.AUTO_BID_AUTHORIZED) %>"
                ...
                <ul class="text_list">
                    ...
                    <li>
                        <div class="til">年利率范围：</div>
                        <div class="con">
                            <input name="rateStart" type="text"
                                   class="text_style yhgl_ser w70 border required maxf-size-99.99 minf-size-0.01 lxmin-size"
                                   style="width:85px;" 
                                   value=""/>
                            %- <input name="rateEnd" type="text"
                                      class="text_style yhgl_ser w70 border required maxf-size-99.99 minf-size-0.01 lxmax-size"
                                      mtest="/^\d*(\d|(\.[0-9]{1,2}))$/" mtestmsg="只能有两位小数" value=""
                                      style="width:85px;"/>
                            %
                    ...
                    <li>
                        ...
                        <div class="con">
                            <select name="jkqxStart" class="select_style required"
                                ...
                                <%} %>
                            </select> 月- <select name="jkqxEnd" class="select_style required"
                                                 onchange="vailJkqx()" style="width:97px;">
                            <%
                                for (int i = 1; i <= 36; i++) {
                            %>
                    ...
                    <li class="mctbje">
                        ...
                        <div class="con">
                            <input type="text" name="saveMoney"
                                   class="text_style yhgl_ser border required isint  max-size-<%=userInfo.F06 %>"/>元
    ...
    <script type="text/javascript" src="<%=controller.getStaticPath(request)%>/js/huifu/zdtb.js"></script>
    <script type="text/javascript">
        var postUrl = '<%=controller.getURI(request, ExistRule.class)%>';
        var openUrl = '<%=controller.getURI(request, HFZdtb.class)%>';
        var closeUrl = '<%=controller.getURI(request, HFZdtbClose.class)%>';
        <%-- var updateUrl = '<%=controller.getURI(request, UpdateRule.class)%>'; --%>
        var delUrl = '<%=controller.getURI(request, DeleteRule.class)%>';
        var count = '<%=autoBidSets==null?0:autoBidSets.size()%>';
        $(function () {
            var flag = <%=flag%>;
            if(!flag){
