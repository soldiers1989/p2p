...
@Right(id = "P2P_C_HUIFUDZGL_HKDZGL_QUERY", isMenu = true, name = "还款对账操作", moduleId = "P2P_C_HUIFUDZGL_HKDZGL", order = 2)
            ...
            else if (HuiFuConstants.Common.SUCCESS_CODE.equals(entity.RespCode))
            {
                if (OrderStatus.P.name().equals(entity.TransStat))
                {
                    if (t6501.F03 != T6501_F03.CG && T6501_F11.F == t6501.F11)
                    {
                        ...
                        {
                            logger.error("PaymentQuery.processPost" + e);
                            //购买失败，更新还款计划为未还
                            bidManage.updatePayFailT6252(IntegerParser.parse(ordId));
                            //更新对账状态
